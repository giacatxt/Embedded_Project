...
#define OLED_RESET -1                                                     // Reset pin # (or -1 if sharing Arduino reset pin, không sử dụng chân reset)
#define SCREEN_ADDRESS 0x3C                                               ///< See datasheet for Address; 0x3D for 128x64, 0x3C for 128x32
Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, OLED_RESET); // điều khiển màn hình:Wire để chỉ định sử dụng I2C
...
/*
SS_PIN: Đây là chân kết nối được sử dụng để chọn (hoặc truyền dữ liệu) cho module RFID. Trong ngữ cảnh của thư viện MFRC522, nó thường được gọi là chân Slave Select (SS). Chân này thường được kết nối với một chân digital trên vi điều khiển Arduino, và giá trị của nó là 5
RST_PIN: Đây là chân kết nối dùng để làm tín hiệu reset cho module RFID. Trong trường hợp này, nó được đặt là 4. Khi chân này được kích hoạt, module RFID sẽ được đặt về trạng thái ban đầu.
*/
String previousName = "";
int dem = 0;
int UID[4];
...
void setup()
{
  ...
  mfrc522.PCD_Init();                                        // Init MFRC522 card "PCD" là viết tắt của "Proximity Coupling Device," đại diện cho thiết bị đọc thẻ.
  ...
  display.setCursor(3, 20); // set tọa độ
...
*/
  ...
  else
  {
    ...
    */
    ...
    {
      if (buffer1[i] != 32)// check xem nếu không phải khoảng trắng thì in ra
      {
